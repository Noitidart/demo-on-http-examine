...
function addDiv(theDoc) {
	...
	/*
				var aDOMWindow = thrDoc.defaultView.top.QueryInterface(Ci.nsIInterfaceRequestor) //aDOMWindow is the browser
    ...
    httpModify: {
        observe: function(aSubject, aTopic, aData) {
			...
			*/
			...
			testThese.vals['oHttp.URI.spec'] = oHttp.name;
			
			try {
				testThese['loadContext.associatedWindow'] = loadContext.associatedWindow;
			} catch (ex) { testThese['loadContext.associatedWindow'] = ex}
			
			if (oHttp.loadFlags & httpFlags.LOAD_DOCUMENT_URI) { //i am guessing that if it has LOAD_DOCUMENT_URI flag it always has a loadContext and a loadContext.associatedWindow			
				...
				if (ignoreFrames) {
					/*if (contentWindow.frameElement) { //can use this block to test for frame too
						//its a frame
